...
export function BillingView({ mode }: BillingViewProps) {
    ...
    useEffect(() => {
        const currentBalance = transactions.reduce((acc, tx) => {
            // Credit (positive) increases balance, Debit (negative) decreases it
            // Assumption: 'amount' in DB for credit is positive, for debit is negative?
            // Let's verify: In create_transactions_table: amount check? No check.
            // Standard practice: Credit = +100, Debit = -50.
            // If debits are stored as negative: acc + tx.amount
            // If debits are stored as positive but type 'debit':
            // Let's assume standard signed values or handle by type.
            // Let's use TYPE to be sure.
        ...
        try {
            ...
            if (cardPortion > 0 && paymentIntentId) {
                ...
                await supabase.from('transactions').insert({
                    ...
                    currency: 'EUR',
                    metadata: { payment_source: 'card', stripe_payment_intent: paymentIntentId, is_system: true }
                })
    ...
    const onFundSuccess = async (paymentIntentId: string) => {
        try {
            const amount = Number(fundAmount)
            const { error } = await supabase.from('transactions').insert({
                user_id: profile?.id,
            ...
            toast({ title: "Success, but...", description: "Payment was successful, but we had trouble updating your balance.", variant: "destructive" })
        }
    }
                ...
                <Card className="bg-primary/5 border-primary/20 min-w-[300px]">
                    <CardContent className="p-4 flex items-center justify-between gap-4">
                        ...
                        <div className="flex gap-2">
                            <Button size="sm" variant="outline" onClick={() => setIsFundOpen(true)}>
                                ...
                                <Dialog open={isWithdrawOpen} onOpenChange={setIsWithdrawOpen}>
                                    ...
                                    <DialogContent>
                                        <DialogHeader>
                                            ...
                                            <DialogDescription>
                                                Enter the amount you wish to withdraw. Available balance: €{balance.toFixed(2)}
                                            </DialogDescription>
                                        </DialogHeader>
            ...
            <Dialog open={isFundOpen} onOpenChange={setIsFundOpen}>
                <DialogContent className="sm:max-w-[400px]">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Top up your balance using your saved payment method.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-4">
                        {!fundClientSecret ? (
                            <>
                                ...
                                <div className="text-sm text-muted-foreground bg-muted p-3 rounded-md italic">
                                    You will be prompted to enter your card details in the next step.
                                </div>
                            </>
                        ) : (
                            <div className="border rounded-lg p-4 bg-muted/30">
                                <Elements stripe={stripePromise} options={{ clientSecret: fundClientSecret }}>
                                    <StripePaymentForm
                                ...
                                <Button
                                    ...
                                    size="sm"
                                    className="w-full mt-4 text-muted-foreground hover:text-primary"
                                    onClick={() => setFundClientSecret(null)}
                                >
                    ...
                    {!fundClientSecret && (
                        <DialogFooter>
                            ...
                            <Button onClick={handleFundWallet} disabled={funding || !fundAmount}>
                                {funding ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <TrendingUp className="mr-2 h-4 w-4" />}
                                Continue to Payment
                            </Button>
                        </DialogFooter>
                    )}
