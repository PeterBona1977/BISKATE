-- SCRIPT FINAL PARA CORRIGIR ESTRUTURA E POLÍTICAS DE NOTIFICATIONS
-- Corrigido o erro de sintaxe do loop FOR
...
-- Verificar tabela user_feedback
DO $$
DECLARE
    user_column TEXT;
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'user_feedback') THEN
        ...
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_feedback' AND column_name = 'user_id') THEN
            user_column := 'user_id';
        ELSIF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_feedback' AND column_name = 'author_id') THEN
            user_column := 'author_id';
        ELSE
            RAISE NOTICE '❌ Nenhuma coluna de usuário encontrada na tabela user_feedback';
            RETURN;
        END IF;
        ...
        RAISE NOTICE '✅ Políticas de user_feedback criadas usando coluna %', user_column;
    END IF;
END $$;
...
AND tablename IN ('profiles', 'gigs', 'gig_responses', 'notifications', 'moderation_alerts', 'user_feedback')
